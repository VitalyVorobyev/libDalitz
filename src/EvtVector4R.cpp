...
EvtVector4R::EvtVector4R(const double& e,const double& p1,const double& p2,const double& p3){
  v[0]=e; v[1]=p1; v[2]=p2; v[3]=p3;
}

double EvtVector4R::mass() const{
  double m2=v[0]*v[0]-v[1]*v[1]-v[2]*v[2]-v[3]*v[3];
  return m2>0.0 ? sqrt(m2) : 0;
}

EvtVector4R rotateEuler(const EvtVector4R& rs,const double& alpha,const double& beta,const double& gamma){
  EvtVector4R tmp(rs);
...
void EvtVector4R::applyRotateEuler(const double& phi,const double& theta,const double& ksi){
  double sp = sin(phi);
  double st = sin(theta);
  double sk = sin(ksi);
  double cp = cos(phi);
...
double EvtVector4R::dot (const EvtVector4R& p2 )const{
  ...
  return v[1]*p2.v[1] + v[2]*p2.v[2] + v[3]*p2.v[3];
} //dot
