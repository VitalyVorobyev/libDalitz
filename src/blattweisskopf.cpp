...
const int BlattWeisskopf::FFMeson = 0;
const int BlattWeisskopf::FFResonance = 1;

BlattWeisskopf::BlattWeisskopf(const int spin, const double& p0,
                               const int type) :
    FormFactor(type == FFMeson ?
                   m_r_meson : m_r_resonance, p0),
    m_spin(spin), m_type(type), m_F0(compute(p0)) {}
...
double BlattWeisskopf::compute(const double& p) const {
    ...
    case 4:
        ...
        break;
    case 5:
        denom = zSq*(zSq*(zSq*(zSq*(zSq + 15.0) + 315.0) + 6300.0) + 99225.0)
                + 893025.0;
        break;
    default:
        std::cout << "BlattWeisskopf::compute: wrong spin " << m_spin
                  << std::endl;
    }
    return std::sqrt(1./denom);
}
