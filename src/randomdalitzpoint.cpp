...
RandomDalitzPoint::RandomDalitzPoint(const double& mmo, const double& mca,
    ...
    DalitzPhaseSpace(mmo, mca, mcb, mcc), m_max_tries(1e6) {
    init();
}
    ...
    do {
        *mABsq = (*unifAB)(*re); *mACsq = (*unifAC)(*re);
        if (++ntries > m_max_tries) {
            cerr << "GetStripPoint: Tries limit exceeded" << endl;
            return -1;
        }
...
void RDPoint::GetUnconstrainedPoint(double* mABsq, double* mACsq) const {
    *mABsq = (*unifAB)(*re); *mACsq = (*unifAC)(*re);
}
