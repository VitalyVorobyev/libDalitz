...
public:
  ...
  const EvtResonance2* Res(const int resn) {return resn<(int)m_res_v.size() ? m_res_v[resn] : m_res_v[0];}
  ...
  void SetTheta(const int resn, const double& a) {if(resn<(int)m_res_v.size()) m_res_v[resn]->SetTheta(a); return;}
