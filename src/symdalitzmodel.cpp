...
SymDalitzModel::SymDalitzModel(const double& mmo, const double& mcha,
                               const double& mchb, const double& delmin,
                               const double& delmax) :
    DalitzModel(mmo, mcha, mchb, mchb),
    del_min(delmin), del_max(delmax), nbins(8) {}
...
int SymDalitzModel::GetBin(const double& mp, const double& mm) {
    ...
    cout << "GetBin: delta = " << delt << ", mp: " << mp
         << ", mm: " << mm << endl;
    return 0;
}

void SymDalitzModel::PPbarDelta(const double& mp, const double& mm,
